...
get_zate() {
	...
	then
		cd ~
		echo "Installing ssh keys etc, get ready to put in your passphrase, hint, it's that massive 38 character thing you love"
		curl -s https://files.zate.org/ssh.tar.gz.enc | openssl enc -aes-256-cbc -d | tar -zxvf -
		git clone -q git@github.com:Zate/blog.git
	fi
}

sudo apt update
sudo apt -y
sudo apt -y install wget curl bzip2
cd ~
get_go
cd ~
